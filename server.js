...
app.post("/generate-handover", async (req, res) => {
  try {
    ...
    // ✅ Fixed ASCII filename (most compatible with GPT Actions)
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="BiteSize-Handover.docx"'
    );
...
app.post("/generate-handover", async (req, res) => {
  try {
    ...
    // ✅ Return docx without Content-Disposition header (avoids ERR_INVALID_CHAR)
    res.setHeader(
      "Content-Type",
      "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
    );
