...
app.post("/generate-handover", async (req, res) => {
  try {
    ...
    // Fixed safe ASCII filename (avoid header encoding issues entirely)
...
app.post("/generate-handover", async (req, res) => {
  try {
    ...
    // Quick log to confirm Action is reaching Render
    console.log("generate-handover hit:", {
      business_name: d?.business_name,
      client_name: d?.client_name,
    });
    ...
    // ✅ Headers that help Actions handle the binary response cleanly
    res.status(200);
    res.setHeader(
      "Content-Type",
      "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
    ...
    // Send as a single response (avoids stream glitches)
